  ...
  class AudioTrans {
    ...
    async concatMP3(opts = {}) {
      try {
        ...
        var bashCmd = [`ffmpeg -y`, `-f concat -safe 0`, `-i ${inpath}`];
        album && bashCmd.push(`-metadata album="${album}"`);
        album_artist &&
          bashCmd.push(`-metadata album_artist="${album_artist}"`);
    ...
    async concatOpus(opts = {}) {
      try {
        ...
        album_artist &&
          bashCmd.push(`--comment album_artist="${album_artist}"`);
