(function (exports) {
  ...
  class SectionParser {
    ...
    expandableSegments(segments) {
      ...
      if (iEll.length <= 1) {
        throw new Error(`not implemented:` + JSON.stringify(segments[iEll[0]]));
      }
      return iEll;
    }
      ...
      for (let i = 0; i < iEll.length; i++) {
        ...
        if (i === 1) {
          ...
          if (prefix === "") {
            ...
            values[1] = words1.slice(words1.length - words2.length).join(" ");
            prefix = segments[iEll[0]][prop].replace(
              new RegExp(`${values[1]}.*`),
              ""
            );
          }
    ...
    sectionGroup(segments, start, length) {
      ...
      var patLast =
        new SuttaCentralId(segments[start + length - 1].scid).parent.scid + "*";
    ...
    parseExpandableSection(segments) {
      ...
      for (var it = iEll[0]; !phrase && 0 <= --it; ) {
        phrase = Words.commonPhrase(
          segments[iEll[0]][prop],
          segments[it][prop],
          MIN_PHRASE
        );
      }
      ...
      if (!phrase) {
        console.error(
          `no expansion template for alternate:` +
       ...
       */
      var prefix = text1.substring(
        0,
        text1.indexOf(phrase) + phrase.length + 1
      );
      //console.debug(`parseExpandableSection prefix:"${prefix}"`);

      var values = [];
      if (this.areAdjacent(segments, iEll[0], iEll[1], prop)) {
        var { indexes, prefix, values } = this.parseContiguousAlternates(
          segments,
          phrase,
          prefix,
          it,
        ...
        var { indexes, prefix, values } = this.parseAlternatesByPhrase(
          segments,
          phrase,
          prefix
        );
      ...
      if (!RE_ELLIPSIS.test(textLast)) {
        // last value was expanded
