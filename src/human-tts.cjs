(function (exports) {
  ...
  const NOAUDIOPATH = path.join(__dirname, "data", "no_audio.mp3");
  ...
  class HumanTts extends AbstractTTS {
    ...
    synthesizeSegment(opts = {}) {
      ...
      var { voice, mj, noAudioPath } = this;
      soundStore = soundStore || this.soundStore;
      if (soundStore == null) {
        return Promise.reject(
          new Error(`synthesizeSegment() soundStore is required`)
        );
      }
      scAudio = scAudio || this.scAudio;
      if (scAudio == null) {
        return Promise.reject(
          new Error(`synthesizeSegment() scAudio is required`)
        );
      }
