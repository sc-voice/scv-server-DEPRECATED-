  ...
  class S3Bucket {
    constructor(opts = {}) {
      ...
      if (opts.s3 instanceof AWS.S3) {
        ...
        let s3Opts = Object.assign({}, DEFAULT_OPTS.s3, opts.s3);
    ...
    initialize() {
      var { Bucket, LocationConstraint, s3 } = this;
      if (this.initialized) {
        this.info("initialize() redundant (IGNORED)");
          ...
          try {
            ...
            s3.listBuckets(function (err, data) {
              ...
              if (data.Buckets.some((b) => b.Name === Bucket)) {
                that.info(`S3Bucket.initialize() found bucket:${Bucket}`);
                that.initialized = true;
                resolve(that);
              } else {
                that.info(`S3Bucket.initialize() creating bucket:${Bucket}`);
        ...
        (async function () {
          try {
            ...
            s3.createBucket(params, function (err, data) {
              ...
              that.warn(`createBucket() location:${data.Location}`);
              resolve(data);
            });
          } catch (e) {
            reject(e);
      ...
      return new Promise((resolve, reject) => {
        (async function () {
          try {
            ...
            var msgContext =
              `S3Bucket.downloadObject() ` + `${Bucket}/${oname} => ${dstPath}`;
            ostream.on("error", (err) => {
              that.warn(`${msgContext} WARNING`);
              that.warn(err.stack);
              reject(err);
            });
            ostream.on("finish", () => {
              that.info(`${msgContext} OK`);
              resolve({
                Bucket,
                Key: oname,
                dstPath,
