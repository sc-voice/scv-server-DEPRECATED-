typeof describe === "function" &&
  describe("words", function () {
    ...
    it("romanize(text) returns romanized text", function () {
      ...
      should(words.romanize("‘Nandī dukkhassa mūlan’ti—")).equal(
        `${Words.U_LSQUOTE}nandi dukkhassa mulan${Words.U_RSQUOTE}ti${Words.U_EMDASH}`
      );
    });
    it("tokenize(text) handles numbers", function () {
      ...
      var tokens = words.tokenize(
        "8,400,000,000 cars 2,400,000 and 6,000, and 600"
      );
    ...
    it("add(word, language) return IPA for word", function () {
      ...
      should.deepEqual(words.alternates("centre"), ["center", "centre"]);
    ...
    it("commonPhrase(a,b,minLength) returns longest common word left sequence", function () {
      var minLength = 1;
      should(Words.commonPhrase(`b1 b2 v0aaa v1b`, `v0aaa v2b`, 4)).equal(
        `v0aaa`
      );
    ...
    it("partitions list of phrases", function () {
      ...
      for (var i = 0; i < lines.length; i++) {
        for (var j = i + 1; j < lines.length; j++) {
          var phrase = Words.commonPhrase(lines[i], lines[j], minLength);
