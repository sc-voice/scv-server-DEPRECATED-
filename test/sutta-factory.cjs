typeof describe === "function" &&
  describe("sutta-factory", function () {
    const should = require("should");
    const fs = require("fs");
    const path = require("path");
    const { MLDoc } = require("scv-bilara");
    ...
    it("TESTTESTloadSutta(...) automatically sections a Sutta", async () => {
      ...
      should(sutta.sections[0].segments.length).equal(2);
      should(sutta.sections[0].segments[0].en).match(/Long Discourses 2/);
      should(sutta.sections[0].segments[1].en).match(
      ...
      should(sutta.sections[1].segments[1].en).match(/So I have heard/);
    });
