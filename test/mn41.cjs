typeof describe === "function" &&
  describe("mn41", function () {
    ...
    it("TESTTESTparseSutta(sutta) parses mn41 structure", async()=>{
      var segments = [
        {
          // section[0]
          scid: "mn41:0.1",
      ...
      should.deepEqual(
        sections.map((section) => section.expandable),
        [false, false, false]
      );
      ...
      should.deepEqual(
        parsedSutta.sections[0].segments,
        segments.slice(0, 1)
      );
      should.deepEqual(
        parsedSutta.sections[1].segments,
        segments.slice(1, 13)
      );
      should.deepEqual(
        parsedSutta.sections[2].segments,
        segments.slice(13, 14)
      );
    ...
    it("TESTTESTparseSutta(sutta) parses mn41", async()=>{
      ...
      should.deepEqual(
        sections.map((section) => section.segments.length),
        [2, 58, 56, 6]
      );
      var sectSegs = sections.reduce((acc, section) => {
        section.segments.forEach((seg) => acc.push(seg));
